    ...
    <view class="cCon">
      <view class="outInBlock">
        ...
        <scroll-view scroll-y="true" scroll-x="true" class="xscoll">
          <view class="time-box">
            <view id="left" class="cheader" :style="{ width: leftFixWidth }">
              <view class="left">
                <view class="text"></view>
              </view>
            </view>
            <view id="title" class="title" :style="{ left: leftFixWidth }">
              <view v-for="(head, index) in tableHeader" :key="index" class="time-block" :style="headerStyle">
                <slot name="header" :header="head"></slot>
                {{ !$slots.header ? (headerKey ? head[headerKey] : head) : '' }}
              </view>
            </view>
          </view>
          <view class="left" :style="{ width: leftFixWidth }">
            <view class="classify">
              <view
                ...
                :key="index"
                class="block"
                :style="{ height: classifyDom[index] + 'px' }"
              >
                <slot name="classifyItem" :classify-item="item"></slot>
                {{ !$slots.classifyItem ? (classifyDataKey ? item[classifyDataKey] : item) : '' }}
              </view>
            </view>
            <view class="classifyBox">
              <view v-for="(item, n) in tableContentLeftFixed" :id="getdom(n, item)" :key="n" class="leftbox">
                <view class="outInTitle" :style="leftFixStyle">
                  <slot name="leftFix" :left-fix="item"></slot>
                  {{ !$slots.leftFix ? (leftFixKey ? item[leftFixKey] : item) : '' }}
                </view>
              </view>
            </view>
          </view>
          <view class="right-box">
            <view v-for="(item, n) in tableContent" :key="n" class="right" :style="{ height: domEles[n] + 'px' }">
              <view class="bot">
                <view v-for="(data, i) in item[contentKey]" :key="i" class="infoNum" :style="contentStyle">
                  <slot name="content" :content="data"></slot>
                  {{ !$slots.content ? (unitKey ? data[unitKey] : data) : '' }}
                </view>
              </view>
            </view>
          </view>
  ...
  watch: {
      ...
      handler: function (val) {
        ...
        for (let i = 0; i < keysArray.length; i++) {
          for (let j = 1; j <= this.domIDs[keysArray[i]]; j++) {
            ...
            self.classifyDom[i]
              ? self.$set(self.classifyDom, i, self.classifyDom[i] + val[n])
              : self.$set(self.classifyDom, i, val[n])
            n++
          }
        }
      },
    ...
    tableContentLeftFixed: {
      handler: function () {
        let self = this
        let i
        this.tableContentLeftFixed.forEach((data) => {
